...
find_ICD10_filter <- function(data, search_pattern, new_col_name) {
    ...
    matching_columns <- grep("^diagnoses_icd10_f41270_\\d+_\\d+$", column_names)
...
## Create funktion to make a new_col_name = 1 if an individual has a specific drug code. If multiple search_integers, provide as a vector c(x, y). new_col_name has to be in "".
...
## NEW FUNCTION FOR BASELINE ONLY: Create funktion to make a new_col_name = 1 if an individual has a specific drug code. If multiple search_integers, provide as a vector c(x, y). new_col_name has to be in "".

find_drug_filter_bs <- function(data, search_integers, new_col_name) {
    # Define the pattern to match column names
    pattern <- "^treatmentmedication_code_f20003_0_\\d+$"
